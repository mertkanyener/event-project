...
public interface EventRepository extends JpaRepository<Event, Long> {
    ...
    @Query("select event from Event event where event.date between current_date and :monthEnd")
    List<Event> findTillEndOfMonth(@Param("monthEnd")LocalDate monthEnd);
    @Query("select event from Event event where event.date between :montStart and :monthEnd")
